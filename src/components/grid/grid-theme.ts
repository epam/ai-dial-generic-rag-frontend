...
/**
 * DIAL ag-grid theme, reproducing the look DialGrid applied. Values are `var(--token, #fallback)`
 * strings, so the grid uses the live DIAL theme variables when embedded in DIAL Admin and the dark
 * fallbacks standalone. This is the ag-grid Theming API — do NOT also import the legacy
 * `ag-grid-community/styles/*.css` (mixing the two triggers ag-grid error #239).
 * @param isDark - Whether the resolved DIAL theme is dark (see `ThemeContext`'s `isDarkTheme`).
 */
export function getDialGridTheme(isDark: boolean) {
  return themeBalham
